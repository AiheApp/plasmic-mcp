...
export class PlasmicClient {
  ...
  private interpret(r: RawResult, ctx: string): unknown {
    ...
    if (!isJson) {
      ...
      if (r.status < 200 || r.status >= 300) {
        throw new PlasmicError(
          `${ctx} failed: HTTP ${r.status}`,
          r.status,
          r.text.slice(0, 500),
          "http"
        );
      }
      return r.text; // 2xx non-JSON (rare)
    }
  ...
  /** Idempotent: concurrent callers share one login. */
  async login(): Promise<void> {
    if (this.loginInFlight) return this.loginInFlight;
    this.loginInFlight = (async () => {
      await this.fetchCsrf(); // anon session + token
      const r = await this.send("POST", "/api/v1/auth/login", {
        body: { email: this.cfg.email, password: this.cfg.password },
        csrf: true,
      });
      const json = this.interpret(r, "login") as {
        status?: boolean;
        reason?: string;
      };
      if (json?.status !== true) {
        throw new PlasmicError(
          `login failed: ${json?.reason ?? "unknown"}`,
          r.status,
          undefined,
          "auth"
        );
      }
      await this.fetchCsrf(); // refresh token for the (possibly regenerated) authed session
      this.authed = true;
    })();
    try {
  ...
  async request<T = unknown>(
    ...
    let r = await this.send(method, path, {
      body: opts.body,
      csrf: mutating,
    ...
    const stale =
      r.status === 401 || (r.status === 403 && /csrf/i.test(r.text));
    if (stale) {
      this.authed = false;
      await this.login();
      r = await this.send(method, path, {
        body: opts.body,
        csrf: mutating,
        timeoutMs: opts.timeoutMs,
      });
  ...
  delete<T = unknown>(path: string, timeoutMs?: number): Promise<T> {
    return this.request<T>("DELETE", path, { timeoutMs });
  }
}
