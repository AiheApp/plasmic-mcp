...
export interface ScreenshotResult {
  ...
  path: string;
  page: { uuid: string; name: string; path: string | null };
  frame: { index: number; width: number; height: number; count: number } | null;
  /** On-screen capture size in px (zoom-dependent for artboard captures). */
...
/**
 * Capture what Studio ACTUALLY renders for a page — the anti-"silent absence"
 * verification: REST success tells you nodes exist; this tells you they show.
 * Opens a fresh Studio session, switches to the page's dedicated arena, zooms
 * to fit the chosen frame, and screenshots that artboard iframe (or the whole
 * Studio window with fullStudio).
 */
export async function canvasScreenshotOp(
  client: PlasmicClient,
  opts: ScreenshotOpts
  ...
  const outPath =
    opts.outPath ??
    join(tmpdir(), `plasmic-canvas-${opts.projectId}-${Date.now()}.png`);

  const session: StudioSession = await getDriver(client).openStudio(opts.projectId, {
    waitMs,
  });
  try {
    // A blocking modal can cover the canvas — report it, and for artboard
    // captures fail loud (the screenshot would show the modal, not the page).
    const blockingModal = await detectBlockingModal(session.studioFrame);

    // Switches Studio to the page's dedicated arena and waits for a live
    // ViewCtx; throws CANVAS_NO_FRAME for arena-less pages (run
    // plasmic_repair_page_arenas to fix those).
    const target = await ensurePageArena(session.studioFrame, opts.page, waitMs);

    if (opts.fullStudio) {
      await sleep(2000); // let the arena paint
      await session.page.screenshot({ path: outPath });
      const vp = session.page.viewportSize();
      return {
        success: true,
        path: outPath,
        page: { uuid: target.uuid, name: target.name, path: target.path },
        frame: null,
        capturedWidth: vp?.width ?? 0,
        capturedHeight: vp?.height ?? 0,
        blockingModal,
        durationMs: Date.now() - started,
      };
    }
    ...
    if (blockingModal) {
      throw new CanvasError(
        "BLOCKING_MODAL",
        `Studio is blocked by a modal — an artboard capture would show the modal, not the page: ` +
          `"${blockingModal.slice(0, 200)}". Resolve it in Studio (see the runbook) or pass ` +
          `fullStudio:true to capture the blocked state as evidence.`,
        { blockingModal }
      );
    }

    // Pick the arena frame and zoom to fit it (deterministic capture scale).
    const frameInfo = (await session.studioFrame.evaluate(
      ({ uuid, frameIndex }) => {
        const sc = (window as any).dbg.studioCtx;
        const comps: any[] = Array.from(sc.site.components);
        const page = comps.find((comp) => comp.uuid === uuid);
        if (!page) return { ok: false as const, reason: `page ${uuid} vanished` };
        const arena = sc.getDedicatedArena(page);
        const frames: any[] = (arena?.matrix?.rows ?? []).flatMap((r: any) =>
          (r.cols ?? []).map((cell: any) => cell.frame)
        );
        if (!frames.length) return { ok: false as const, reason: "no frames" };
        const idx = Math.min(frameIndex, frames.length - 1);
        const f = frames[idx];
        try {
          sc.tryZoomToFitFrame(f, 1);
        } catch {
          /* capture proceeds at the current zoom */
        }
        return {
          ok: true as const,
          uid: String(f.uid),
      ...
      { uuid: target.uuid, frameIndex: opts.frameIndex ?? 0 }
    )) as
      | { ok: false; reason: string }
      | { ok: true; uid: string; index: number; width: number; height: number; count: number };

    if (!frameInfo.ok) {
      throw new CanvasError("CANVAS_NO_FRAME", `cannot capture: ${frameInfo.reason}`, {
        page: target,
      });
    }

    // The iframe gains data-test-frame-uid only once its content is LOADED —
    // polling for the selector doubles as the render-readiness gate.
    const selector = `iframe[data-test-frame-uid="${frameInfo.uid}"]`;
    const deadline = Date.now() + waitMs;
    let el = await session.studioFrame.$(selector);
    while (!el && Date.now() < deadline) {
      await sleep(500);
      el = await session.studioFrame.$(selector);
    }
    if (!el) {
      throw new CanvasError(
        "CANVAS_NOT_READY",
        `artboard iframe (frame uid ${frameInfo.uid}) did not finish loading within ${waitMs}ms`,
        { page: target, frame: frameInfo }
      );
    }
    await sleep(1500); // paint settle after zoom-to-fit
    ...
    return {
      ...
      capturedWidth: Math.round(box?.width ?? 0),
      capturedHeight: Math.round(box?.height ?? 0),
      blockingModal,
      durationMs: Date.now() - started,
    };
  } finally {
...
export const canvasTools: ToolDef[] = [
  ...
  defineTool({
    name: "plasmic_canvas_screenshot",
    description:
      "Screenshot what Studio ACTUALLY renders for a page (anti-'silent absence' verification: " +
      "REST success says nodes exist, this says they SHOW). Opens headless Studio, switches to " +
      "the page's dedicated arena, zooms to fit the chosen frame (frameIndex, 0 = desktop) and " +
      "captures that artboard to a PNG (outPath or a temp file; path returned). fullStudio:true " +
      "captures the whole Studio window instead. Fails with CANVAS_NO_FRAME on arena-less pages " +
      "(fix with plasmic_repair_page_arenas) and BLOCKING_MODAL when a modal covers the canvas.",
    schema: CanvasScreenshotInput,
    handler: (client, args) => canvasScreenshotOp(client, args),
  }),
