...
import {
  ...
  type TokenSpec,
} from "../clients/write.js";

function makeWriteClient(projectId: string, secretToken?: string): PlasmicWriteClient {
  const tok = secretToken ?? env.projectSecretToken;
  if (!tok) {
    throw new Error(
      "The Write API requires a secret project token. " +
...
const componentBodySchema: z.ZodType<ComponentBody> = z.lazy(() =>
  z.object({
    type: z
      .string()
      .describe(
        'Element type. Common values: "hbox" (horizontal flex), "vbox" (vertical flex), ' +
...
export function registerWriteTools(server: McpServer) {
  server.tool(
    "write_create_component",
    "Create a new page or component in Plasmic via the Write API. " +
    ...
    "Requires PLASMIC_PROJECT_SECRET_TOKEN.",
    {
      ...
      body: componentBodySchema.optional().describe(
        "Element tree for the component body. Example: " +
  ...
  server.tool(
    "write_update_component",
    "Update (upsert) an existing Plasmic component's body via the Write API. " +
    ...
    "Requires PLASMIC_PROJECT_SECRET_TOKEN.",
    {
      ...
      body: componentBodySchema.describe(
        "New element tree body to replace the component with. " +
    ...
    async ({ projectId, secretToken, name, path, byUuid, body }) => {
      ...
      const spec: UpdateComponentSpec = {
        ...(byUuid ? { byUuid } : {}),
        ...(name ? { name } : {}),
        ...(path ? { path } : {}),
        body,
  ...
  server.tool(
    "write_update_tokens",
    "Update design tokens in a Plasmic project via the Write API. " +
    ...
    async ({ projectId, secretToken, tokens }) => {
      const pid = resolveProjectId(projectId);
      const client = makeWriteClient(pid, secretToken);
      const result = await client.updateProject({ tokens: tokens as TokenSpec[] });
      return {
        content: [
          {
            type: "text" as const,
