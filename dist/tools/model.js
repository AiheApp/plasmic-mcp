...
export function registerModelTools(server) {
    server.tool("model_get_component_tree", "Read the full Plasmic project model via the Model API. Returns the complete element tree " +
        ...
        if (componentName) {
            ...
            const match = components.find((c) => c.name?.toLowerCase() === componentName.toLowerCase());
            if (!match) {
                const names = components
                    .map((c) => c.name)
                    .filter(Boolean)
                    .join(", ");
                throw new Error(`Component "${componentName}" not found. Available components: ${names}`);
            }
            result = match;
        }
        return {
