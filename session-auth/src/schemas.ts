...
export const GetPkgPublishStatusInput = z.object({ projectId, pkgVersionId: z.string().min(1, "pkgVersionId is required") }).strict();
export const GetWorkspaceInput = z.object({ workspaceId }).strict();
export const GetDevflagsInput = z.object({}).strict();

export const CreateProjectInput = z.object({ name: z.string().min(1).optional(), workspaceId: z.string().min(1).optional() }).strict();

const hostLessPackageInfo = z.object({
  name: z.string().min(1),
...
export const CreateProjectWithHostlessPackagesInput = z.object({
  name: z.string().min(1, "name is required"),
  hostLessPackagesInfo: z.array(hostLessPackageInfo).min(1, "at least one hostless package is required"),
  workspaceId: z.string().min(1).optional(),
}).strict();

export const CloneProjectInput = z.object({ projectId, name: z.string().min(1).optional(), workspaceId: z.string().min(1).optional() }).strict();
...
export const SetDevflagsInput = z.object({
  key: z.string().min(1, "key is required"),
  value: z.unknown(),
  confirm: z.literal(true, {
    errorMap: () => ({ message: "set_devflags writes a GLOBAL, instance-wide override. Pass confirm:true to proceed." }),
  }).describe("must be true — guards against blind instance-wide writes"),
}).strict();

const grant = z.object({ email: z.string().email(), accessLevel: z.string().min(1), projectId: z.string().min(1).optional(), workspaceId: z.string().min(1).optional(), teamId: z.string().min(1).optional() }).strict();
const revoke = z.object({ email: z.string().email(), projectId: z.string().min(1).optional(), workspaceId: z.string().min(1).optional(), teamId: z.string().min(1).optional() }).strict();

export const GrantRevokeInput = z.object({ grants: z.array(grant).optional(), revokes: z.array(revoke).optional() }).strict();
...
const copilotImage = z.object({ type: z.enum(["png", "jpg", "jpeg", "webp"]), base64: z.string().min(1) }).strict();
