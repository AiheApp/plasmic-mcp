...
export function registerStudioBrowserTools(server) {
    server.tool("studio_get_canvas_state", "Read the current element tree and state from the open Plasmic Studio tab. " +
        "Requires Chrome to be running with --remote-debugging-port and the project open.", { projectId: projectIdParam }, async ({ projectId }) => {
        const pid = resolveProjectId(projectId);
        const state = await withStudioPage(pid, (page) => getCanvasState(page));
        return { content: [{ type: "text", text: JSON.stringify(state, null, 2) }] };
    });
    server.tool("studio_select_element", "Select an element by name in the Plasmic Studio Layers panel.", {
        ...
        return { content: [{ type: "text", text: `Selected element "${elementName}".` }] };
    });
    server.tool("studio_add_element", "Add an element to the canvas using the Plasmic Insert panel. " +
        "Optionally select a target slot/container first.", {
        projectId: projectIdParam,
        elementType: z.string().describe('Type of element to insert. Examples: "Text", "Box", "Button", "Image", "Icon"'),
        targetSlot: z
            .string()
            .optional()
    ...
    server.tool("studio_remove_element", "Remove an element from the canvas. Selects it by name first if provided, " +
    ...
    server.tool("studio_set_props", "Set properties or styles on a canvas element via the Plasmic right-side panel. " +
        ...
        const pid = resolveProjectId(projectId);
        await withStudioPage(pid, (page) => setElementProps(page, props, elementName));
        return {
            content: [
                {
                    type: "text",
    ...
    server.tool("studio_move_element", "Move an element up or down within its parent in the Plasmic Layers panel.", {
        projectId: projectIdParam,
    ...
    server.tool("studio_screenshot", "Take a screenshot of the current Plasmic Studio state and return it as a base64 PNG.", { projectId: projectIdParam }, async ({ projectId }) => {
        const pid = resolveProjectId(projectId);
    ...
    server.tool("studio_navigate", "Navigate the open Plasmic Studio tab to a specific component or page by name/path.", {
        projectId: projectIdParam,
        componentName: z
            .string()
            .optional()
...
//# sourceMappingURL=studio-browser.js.map